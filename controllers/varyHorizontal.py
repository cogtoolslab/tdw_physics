...
class Stability(RigidbodiesDataset):
    ...
    #THIS CONTROLS NUM BLOCKS
        ...
        num_objects = 8 #random.randrange(4,8, 2)
        #precarious = True #determine if blocks should be sampled from "precarious" set
        ...
        for i in range(num_objects):
            ...
            # Add the object.
            scale = 0.23 #random.uniform(0.1, 0.23)
        ...
        a_pos = self.get_random_avatar_position(radius_min= 1.1 * y,
                                                radius_max=1.2 * y,
                                                y_min=y / 4,
                                                y_max=y / 2,
    ...
    def _add_object_to_stack(self, record: ModelRecord, y: float, scale: float) -> List[dict]:
        ...
        commands.extend(self.add_physics_object(record=record,
                                                position={"x": random.uniform(-scale/2.5, scale/2.5),
                                                          "y": y,
