...
class Stability(RigidbodiesDataset):
    ...
    #THIS CONTROLS STABILITY
        ...
        self._stack_type = _StackType.stable#random.choice([st for st in _StackType])
        print(self._stack_type)
    #THIS CONTROLS NUM BLOCKS
        #Change number of blocks
        num_objects = random.randrange(6,10, 2)
        #edit base
        num_base = 4
        ...
        for i in range(num_objects):
            ...
            if self._stack_type == _StackType.stable:
                # Every object except the top is "stable".
                if i < num_objects:
            ...
            if(num_base == 1):
                ...
                commands.extend(self._add_object_to_stack(record=record, x = random.uniform(-0.02, 0.02), y=y, scale=scale))
                # Increment the starting y positional coordinate by the previous object's height.
                y += record.bounds['top']['y'] * scale

            if(num_base == 2):
                if(i==0):
                    #make all base objects the same size
                    scale = 0.23
                    x = scale/2
                    z = random.uniform(-0.02, 0.02)
                    commands.extend(self._add_object_to_stack(record=record, x =x, y=y, z = z, scale=scale))
                    # Increment the starting y positional coordinate by the previous object's height.
                    #y += record.bounds['top']['y'] * scale
                elif(i==1):
                    scale = 0.23
    ...
    def _add_object_to_stack(self, record: ModelRecord, x:float, y: float, z: float, scale: float) -> List[dict]:
        """
...
#if base 1, do normal, if base 2, put side-by-side, if base 4, put in square
#For side-by-side: if i<2, alter x axis to be 1/2width
